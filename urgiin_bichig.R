...
ggplot() +
  ...
  geom_vline(xintercept=seq(
    as.numeric(as.Date("1800-01-01")), as.numeric(as.Date("2030-01-01")), by=10*365),
  ...
  labs(title=expression(atop(
    "Ganjuur's family tree",
    atop(italic("compiled by Namkhainyam, visualized by Nicolas Jérémie Kruchten"), ""))))
dev.off()
